...
namespace PPF2DataEditor
{
    public static class MRG
    {
        ...
        public static bool Extract(Stream input, string outPath)
        {
            ...
            // Check to see if this is a MRG
            if (!(input.ReadByte() == 'M' && input.ReadByte() == 'R' && input.ReadByte() == 'G' && input.ReadByte() == '0'))
            {
                return false;
            }
            ...
            for (int i = 0; i < numFiles; i++)
            {
                ...
                input.Position += 4;
                string fname = Extensions.ReadString(input, Encoding.GetEncoding("Shift_JIS"), 32);
